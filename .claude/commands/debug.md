...
- MCP logs: `~/.recurse/logs/mcp-claude-approvals-*.log`
- Combined WUI/Daemon logs: `~/.recurse/logs/wui-${BRANCH_NAME}/codelayer.log`
- First line shows: `[timestamp] starting [service] in [directory]`

**Database**:
- Location: `~/.recurse/daemon-{BRANCH_NAME}.db`
...
- Socket exists: `~/.recurse/daemon.sock`
...
1. Find latest daemon log: ls -t ~/.recurse/logs/daemon-*.log | head -1
2. Find latest WUI log: ls -t ~/.recurse/logs/wui-*.log | head -1
...
1. Connect to database: sqlite3 ~/.recurse/daemon.db
...
**Find Latest Logs**:
```bash
ls -t ~/.recurse/logs/daemon-*.log | head -1
ls -t ~/.recurse/logs/wui-*.log | head -1
```

**Database Queries**:
```bash
sqlite3 ~/.recurse/daemon.db ".tables"
sqlite3 ~/.recurse/daemon.db ".schema sessions"
sqlite3 ~/.recurse/daemon.db "SELECT * FROM sessions ORDER BY created_at DESC LIMIT 5;"
```
